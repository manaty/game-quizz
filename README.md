...
### Quiz show presentation (1.3.0)
Questions enter in sequence; the final seconds pulse gently and answers have distinct reveal animations. The final podium names the winner (including tied leaders), shows player photos, and celebrates with bounded fireworks, confetti and an original synthesized applause/fanfare cue. Animations do not restart on heartbeat updates and honor reduced-motion preferences. Audio requires a browser gesture and can be muted. Native museum hosts own audio through the shared quiz cue module; standalone packages synthesize the same cues locally.
