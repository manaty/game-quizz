...
const noContentPlugin={name:'external-content',setup(b){b.onResolve({filter:/builtins\.js$/},()=>({path:'empty-bank',namespace:'content'}));b.onLoad({filter:/.*/,namespace:'content'},()=>({contents:'export const BUILTIN_QUIZZES={};',loader:'js'}));}};
const manifest=JSON.parse(await readFile('retro-museum.json'));const bundle=async(p,plugins=[])=>(await build({entryPoints:[p],bundle:true,write:false,format:'iife',target:'chrome58',minify:true,plugins:[randomPlugin,...plugins]})).outputFiles[0].text;
...
await writeFile('dist/content-engine.js',await bundle('src/adapter.js',[noContentPlugin]));
